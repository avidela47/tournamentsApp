...
import { useAuth } from "../context/AuthContext.jsx"; // ⬅️ traemos el contexto
...
const MatchPage = () => {
  ...
  const fetchJSON = async (url, options = {}) => {
    const res = await fetch(url, {
      ...
      headers: {
        ...
        Authorization: `Bearer ${auth?.token}`, // ⬅️ enviamos token
        ...options.headers,
      },
    });
  ...
  const handleDelete = async (id) => {
    await fetchJSON(`${API_URL}/matches/${id}`, { method: "DELETE" });
    await loadMatches();
  };
      ...
      {groupedMatches.map((tournament) => (
        <div key={tournament._id} className="mb-10">
          ...
          {tournament.matches.length === 0 ? (
            <p className="text-gray-500 text-center">No hay partidos en este torneo.</p>
          ) : (
            [...new Set(tournament.matches.map((m) => m.jornada))].map((jornada) => (
